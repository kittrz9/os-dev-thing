...
#cd "$ORIGIN_DIR"
python generateFS.py
FS_SIZE="$(python -c "import os; import math; a=os.path.getsize(\"build/fs.bin\");print(math.ceil(a/512))")"
...
dd if=build/fs.bin of=build/os.bin seek="$(echo "512 + $STAGE2_SIZE_BYTES" | bc)" bs=1 count=$FS_SIZE_BYTES
